...
{#each data_grouped as group}
	...
	<div class="bar-group" style:height="{zDomain[1] ? barHeight * 2 : barHeight}px" style:width="calc(100% - {margin}px)">
	{#each group.values as d, i}
		<div class="{i == 0 ? 'bar' : 'bar2'}" style:left="0" style:width="{xScale(d[xKey])}%" style:top="{i == 0 ? '0' : '50%'}" style:height="{zDomain[1] ? "50%" : "100%"}"/>
		<div class="label" class:bold="{i == 0}" style:left="calc({xScale(d[xKey])}% + 2px)" style:top="{i == 0 ? '0' : '50%'}" style:height="50%" bind:clientWidth={d.width}>{formatTick(d[xKey])}{suffix}</div>
	{/each}
	</div>
{/each}
